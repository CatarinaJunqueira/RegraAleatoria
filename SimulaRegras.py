...
def Rc9(Navio, cont):  # This rule fills the ship matrix through a stack score based on the removal priority
    B = len(Navio)  # numero de baias no navio
    L = Navio[0].shape[0]  # numero de linhas no navio
    C = Navio[0].shape[1]  # numero de colunas no navio
    flag = 0
...
# -----------------------------------------------------------------#
# -----------------------------------------------------------------#
def Rc10(Navio, cont):  # Por coluna, de baixo para cima, da esquerda para a direita + a stack score
...
# -----------------------------------------------------------------#
# -----------------------------------------------------------------#
def Rc11(Navio, cont):  # por linha, da direita para a esquerda + a stack score
...
# -----------------------------------------------------------------#
# -----------------------------------------------------------------#
def Rc12(Navio, cont):  # por coluna, da direita para esquerda + a stack score
...
# -----------------------------------------------------------------#
# -----------------------------------------------------------------#
def Rc13(Navio, cont):  # Por linha, de baixo para cima, da esquerda para a direita, intercalando as baias + a stack score
...
# -----------------------------------------------------------------#
# -----------------------------------------------------------------#
def Rc14(Navio, cont):  # Por coluna, de baixo para cima, da esquerda para a direita, intercalando as baias + a stack score
...
# -----------------------------------------------------------------#
# -----------------------------------------------------------------#
def Rc15(Navio, cont):  # por linha, da direita para a esquerda, intercalando as baias + a stack score
...
# -----------------------------------------------------------------#
# -----------------------------------------------------------------#
def Rc16(Navio, cont):  # por coluna, da direita para esquerda, intercalando as baias + a stack score
...
def Rc17(Navio, cont):  # Verificando qual eh a menor coluna, por baia.
...
def Rd1(Navio, porto_dict, P, Rt):
  ...
  # contêineres do porto p e cujos destinos são os portos p+j
            ...
            for c in range(C):  # coluna
                if Navio[b][l][c] != 0 and porto_dict[Navio[b][l][c]] == P:  # se nessa posicao tem um conteiner que vai descer
                    if l != 0:  # se nao estah no topo da coluna, verificar se tem algum conteiner acima bloqueando
                        if Navio[b][l-1][c] != 0:
                            cont_remanejar = Navio[b][:l, c]  # quantidade de elementos acima
                            aux = 1
                            for cont_remanejamento in cont_remanejar:
                                patio_transb.append(cont_remanejamento)
                                Navio[b][l - aux][c] = 0
                                aux += 1
                                N_Rem += 1
    ...
    if len(patio_transb) != 0:
        ...
        Navio = eval(Rt, patio_transb)

    return N_Rem, Navio


def Rd2(Navio, porto_dict, P, Rt):
...
def Rd3(Navio, porto_dict, porto_atual, Rt):
    # Nesta regra, quando o navio chega a um porto p, são removidos todos os
